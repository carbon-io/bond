...
function resolveModule(path, options, mod) { // XXX this is ineffecient. must cache
  ...
  try {
    ...
    if (e instanceof SyntaxError) {
      ...
      error = new Error(syntaxErr.toString())
      error.error = e
      error.stack = e.stack
      error.lineNumber = syntaxErr.line
      error.columnNumber = syntaxErr.column
    } else {
      // XXX the error from module.resolve is not including line number information. 
