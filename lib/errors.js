...
function ResolveModuleSyntaxError(error, filepath) {
  var ecmaVersion = 8
  var syntaxErr = undefined
  do {
    syntaxErr = checkSyntax(
      fs.readFileSync(filepath), filepath, {ecmaVersion: ecmaVersion})
    if (!syntaxErr) {
      ...
      try {
        ...
        if (arrowMessage) {
          ...
          syntaxErr = checkSyntax(
            fs.readFileSync(_filepath), _filepath, {ecmaVersion: ecmaVersion})
        }
      } catch (e) {
        console.debug(e)
      }
    } 
  } while (ecmaVersion-- > 4 && typeof syntaxErr === 'undefined')
